...
import {HttpTelevision, HttpInputAction} from 'http-television';
...
export class InputSourceHandler {
  ...
  private getInputSources(inputs?: HttpInputAction[]) {
    if (inputs) {
      ...
      this.setupInputs([{label: 'Input 1'}]);
    }
  }
    ...
    inputs.forEach((input: HttpInputAction, index: number) => {
      ...
      inputSource
        ...
        .on(
          ...
          this.setConfiguredInputSourceName.bind(this, inputName)
        );
  ...
  private mapInputType(
    type: HttpInputType | string | undefined
  ): CharacteristicValue {
    const Characteristic = this.api.hap.Characteristic;
    switch (type) {
      default:
  ...
  private setActiveIdentifier(
    ...
    try {
      ...
      const activeIndex: number = value as number;
